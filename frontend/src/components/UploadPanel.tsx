  ...
  const handleVideo = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.has_activation) {
        // Auto-paired with TRIBE v2 output — ready to showcase.
        setStatus(`✓ ${res.video_id} paired with TRIBE v2 activation`);
        setNeedsJson(false);
      } else {
        // No cloud activation on file yet — ask for the exported JSON.
  ...
  return (
    <div className="redo-overlay" onClick={onClose}>
      <div className="redo-panel upload-panel" onClick={(e) => e.stopPropagation()}>
        <h2 className="upload-title">Import from Kaggle</h2>
